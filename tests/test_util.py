...
def assert_dict_of_arrays_equal(test_case, dict1, dict2, blacklist):
    ...
    errors = []
    try:
        test_case.assertSetEqual(keys1.difference(blacklist), keys2.difference(blacklist))
    except AssertionError as ex:
        errors.append(str(ex))
        ...
        if key not in blacklist and key in keys1 and key in keys2:
            ...
            try:
                if isinstance(val1, np.ndarray):
                    np.testing.assert_array_almost_equal(val1[~np.isnan(val1)], val2[~np.isnan(val2)],
...
def assert_adata_equal(test_case, data1, data2, obs_blacklist=None, var_blacklist=None, uns_blacklist=None,
    ...
    errors += assert_dict_of_arrays_equal(test_case, data1.uns, data2.uns, uns_blacklist)
    errors += assert_dict_of_arrays_equal(test_case, data1.obsm, data2.obsm, obsm_blacklist)
    errors += assert_dict_of_arrays_equal(test_case, data1.varm, data2.varm, varm_blacklist)
    test_case.assertEqual(len(errors), 0, '\n'.join(errors))
