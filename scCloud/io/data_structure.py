...
class Array2D:
	def __init__(self, barcode_metadata: 'dict or pd.DataFrame' = {}, feature_metadata: 'dict or pd.DataFrame' = {}, matrix: 'csr_matrix' = csr_matrix((0, 0)), metadata: dict = {}):
		self.barcode_metadata = barcode_metadata if isinstance(barcode_metadata, pd.DataFrame) \
								else pd.DataFrame(barcode_metadata)
		self.feature_metadata = feature_metadata if isinstance(feature_metadata, pd.DataFrame) \
		...
		if self.feature_metadata.shape[0] != self.matrix.shape[1]:	
			raise ValueError("Wrong number of features : matrix has {} features, features file has {}".format(self.matrix.shape[1], self.feature_metadata.shape[0]))
	...
	def list_metadata_keys(self, type: str = 'barcode') -> List[str]:
		...
		"""
		if type == 'barcode':
			return [self.barcode_metadata.index.name] + self.barcode_metadata.columns.tolist()
		elif type == 'feature':
			return [self.feature_metadata.index.name] + self.feature_metadata.columns.tolist()
		else:
			list(self.metadata)
	...
	def get_metadata(self, name: str, type: str = 'barcode') -> List[object]:
		...
		"""
		if type == 'barcode':
			return self.barcode_metadata.index if name == 'barcodekey' else self.barcode_metadata[name].values
		elif type == 'feature':
			return self.feature_metadata.index if name == 'featurekey' else self.feature_metadata[name].values
		else:
	...
	def write_to_hdf5(self, keyword: str, hd5_out: 'File') -> None:
		...
		hd5_out.create_carray(out_group, 'shape', obj = (N, M)) # store as feature by barcode instead
		# write barcode_metadata
		outgb = hd5_out.create_group('/' + keyword, '_barcodes')
		hd5_out.create_carray(outgb, self.barcode_metadata.index.name, obj = self.barcode_metadata.index.values.astype('S')) # encode into binary strings
		for col in self.barcode_metadata:
			kind = self.barcode_metadata[col].dtype.kind
			if  kind == 'U' or kind == 'O':
				hd5_out.create_carray(outgb, col, obj = self.barcode_metadata[col].values.astype('S')) # encode into binary strings
			else:
		...
		hd5_out.create_carray(outgb, self.feature_metadata.index.name, obj = self.feature_metadata.index.values.astype('S')) # encode into binary strings
...
def polish_featurename(feature_names: List[str], feature_keys: List[str], genomes: List[str]) -> List[str]:
	...
	for i in range(len(feature_names)):
		...
		if idn > 0:
			feature_names[i] = feature_names[i] + ".{}".format(idn)
...
class MemData:
	...
	def restrain_keywords(self, keywords: str) -> None:
		...
		if len(invalid_set) > 0:
			raise ValueError('Keywords {} do not exist.'.format(','.join(list(invalid_set))))
	...
	def convert_to_anndata(self, concat_matrices = False) -> 'AnnData or List[AnnData]':
		...
		for genome in genomes:
			...
			if not concat_matrices or len(genomes) == 1:
				...
				feature_names, feature_keys = polish_featurename(var_dict.pop('featurename'), array2d.feature_metadata.index.values, [genome])
		...
		if len(results) == 0:
			...
			feature_names, feature_keys = polish_featurename(var_dict.pop('featurename'), fdf.index.values, genomes)
			var_dict['var_names'] = feature_names
			var_dict['gene_ids'] = feature_keys
