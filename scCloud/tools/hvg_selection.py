...
def select_hvg_scCloud(
    data, consider_batch, n_top=2000, span=0.02, plot_hvg_fig=None, benchmark_time=False
):
    ...
    if plot_hvg_fig is not None:
        plot_hvg(
            mean, var, lobj.outputs.fitted_values, hvg_index, plot_hvg_fig + ".hvg.pdf"
        )
...
def select_hvg_seurat_single(
    X, n_top=None, min_disp=0.5, max_disp=np.inf, min_mean=0.0125, max_mean=7
):
    X = X.copy().expm1()
    mean = X.mean(axis=0).A1
    m2 = X.power(2).sum(axis=0).A1
...
def select_hvg_seurat_multi(X, channels, cell2channel, n_top, n_jobs=1):
    ...
    res_arr = np.array(
        Parallel(n_jobs=n_jobs)(
            delayed(select_hvg_seurat_single)(Xs[i], n_top)
...
def select_highly_variable_genes(
    ...
    print(
        "batch_correction.select_highly_variable_genes done. Time spent = {:.2f}s.".format(
            end - start
        )
    )
