...
def collect_contingency_table(data, X, labels="louvain_labels"):
    if data.obs[labels].dtype.name != "category":
        print(
            "Warning: {} is not categorical, scCloud will convert it to categorical.".format(
                labels
            )
        )
...
def collect_stat_and_t_test(
    data, X, clusts=None, labels="louvain_labels", n_jobs=1, temp_folder=None
):
    start = time.time()
    ...
    result_list = Parallel(n_jobs=n_jobs, max_nbytes=1e7, temp_folder=temp_folder)(
        delayed(calc_stat_and_t)(
            i,
            clusts[i],
            data.obs[labels][mask].values,
...
def fisher_test(
    ...
    result_list = Parallel(n_jobs=n_jobs, max_nbytes=1e7, temp_folder=temp_folder)(
        delayed(calc_fisher)(i, clusts[i], data.var_names.values, ct, total)
        for i in range(clusts.size)
    )
...
def format_short_output_cols(df, cols_short_format):
    ...
    cols_short_format_idx = [
        df.columns.get_loc(c) for c in df.columns if c in cols_short_format
    ]
...
test2fields = {
    "t": ["t_pval", "t_qval"],
    "fisher": ["fisher_pval", "fisher_qval"],
...
def write_results_to_excel(output_file, df, alpha=0.05):
    ...
    tests = [
        x
        for x in ["t", "fisher", "mwu"]
        if "{0}_qval_{1}".format(x, clusts[0]) in df.columns
    ]
    ...
    for clust_id in clusts:
        ...
        df_up.sort_values(
            by="auc" if has_roc else "WAD_score", ascending=False, inplace=True
        )
        ...
        df_down.sort_values(
            by="auc" if has_roc else "WAD_score", ascending=True, inplace=True
        )
        # format output as excel table
        ...
        if len(df_up.index) > 0:
            worksheet.add_table(
                ...
                0,
                len(df_down.index),
                len(df_down.columns) - 1,
                {
                    "data": np.array(df_down),
                    "style": "Table Style Light 1",
                    "first_column": True,
                    "header_row": True,
...
def __markers(
    ...
    de_results.extend(
        collect_stat_and_t_test(
            data, X, labels=labels, n_jobs=n_jobs, temp_folder=temp_folder
        )
    )
    ...
    if run_fisher:
        ...
        de_results.extend(
            fisher_test(data, X, labels=labels, n_jobs=n_jobs, temp_folder=temp_folder)
        )

    if run_mwu:
        print("Begin Mann-Whitney U test.")
        de_results.extend(
            mwu_test(data, X, labels=labels, n_jobs=n_jobs, temp_folder=temp_folder)
        )
