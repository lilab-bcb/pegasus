...
def de_analysis(
    ...
    """Perform Differential Expression (DE) Analysis on data. Mann-Whitney U test and AUROC are calculated by default. Welch's T test and Fisher's Exact test are optionally.
...
def markers(
    ...
    """ Extract DE results from de_key and return as a human readible dictionary of pd.DataFrames.
...
def write_results_to_excel(
    results: Union[Dict[str, Dict[str, pd.DataFrame]], Dict[str, Dict[str, Dict[str, pd.DataFrame]]]], output_file: str, ndigits: int = 3
) -> None:
    """ Write results into Excel workbook.

    Parameters
    ----------
    results: ``Dict[str, Dict[str, pd.DataFrame]]`` or ``Union[Dict[str, Dict[str, pd.DataFrame]], Dict[str, Dict[str, Dict[str, pd.DataFrame]]]]``
