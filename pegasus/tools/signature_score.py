...
def _estimate_null(maxvalues: List[float], right_cutoff: int = 0.2) -> List[bool]:
    ...
    kde_f = gaussian_kde(maxvalues)
    x = np.linspace(maxvalues.min(), min(maxvalues.max(), right_cutoff))
    y = kde_f(x)
    # detect peaks
    peak_pos = []
    for i in range(1, x.size - 1):
        if y[i] > y[i - 1] and y[i] > y[i + 1]:
            peak_pos.append(i)
    #
    assert len(peak_pos) > 0
    if len(peak_pos) > 2:
        logger.warning("Interesting! We detected more than 2 peaks. If you have time, please contact Cumulus/Pegasus team to investigate this case.")
    #
    if len(peak_pos) == 1:
        mu = x[peak_pos[0]]
    else:
        ords = np.argsort(y[peak_pos])[::-1]
        mu = (x[peak_pos[ords[0]]] + x[peak_pos[ords[1]]]) / 2.0
    #
    idx = maxvalues < mu
    s = idx.sum()
    if s > 0:
        std = np.sqrt(((maxvalues[idx] - mu) ** 2).sum() / s)
        return maxvalues <= mu + std * 3.0 # 3 sigma rule
    else:
        return np.zeros(maxvalues.size, dtype = bool)
...
def calc_signature_score(
    ...
    signatures: ``Dict[str, List[str]]`` or ``str``
        ...
        Pegasus also provide 4 default signature panels for each of human and mouse. They are ``cell_cycle_human``, ``gender_human``, ``mitochondrial_genes_human`` and ``ribosomal_genes_human`` for human; ``cell_cycle_mouse``, ``gender_mouse``, ``mitochondrial_genes_mouse`` and ``ribosomal_genes_mouse`` for mouse.
            ...
            * ``ribosomal_genes_human`` contains one signature, ``ribo_genes``, which are ribosomal genes from both large and small units.

            * ``cell_cycle_mouse``, ``gender_mouse``, ``mitochondrial_genes_mouse`` and ``ribosomal_genes_mouse`` are the corresponding signatures for mouse. Gene symbols are directly translated from human genes.
    ...
    if isinstance(signatures, str):
        ...
        if sig_string in predefined_signatures:
            ...
            if sig_string.startswith("cell_cycle"):
                ...
                null_idx = _estimate_null(maxvalues)
                cycle_idx = ~null_idx
