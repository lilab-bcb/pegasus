...
def aggregate_10x_matrices(csv_file, genome, restrictions, attributes, output_name):
	...
	df.loc[attributes].to_csv("{output_name}.attr.csv".format(output_name = output_name))

	tot = 0
	out_hd5 = None
	for sample_name, row in df.iterrows():
		input_hdf_file = "{location}/{sample}/filtered_gene_bc_matrices_h5.h5".format(location = row['Location'], sample = sample_name)
		channel = load_h5_file(input_hd5_file, genome)
	...
	write_h5_file("{output_name}_10x.h5".format(output_name = output_name), out_hd5, genome)
	print("Generated {} from {} files.".format(output_file, tot))

def add_attribute(attr_file, attribute_string):
	df = pd.read_csv(attr_file, header = 0, index_col = 0, dtype = str)
	attr, stri = attribute_string.split(':')
	assert attr not in df.columns
	values = stri.split('=')
	if len(values) == 1:
		df[attr] = values[0]
	else:
		assert values[0] in df.columns
		df[attr] = (df[values[0]] == values[1]).astype(int)
	df.to_csv(attr_file) 
...
def merge_10x_matrices(input_names, rep_syms, genome, attributes, output_name):
	...
	for i, input_name in enumerate(input_names):
		input_hdf_file = "{input_name}_10x.h5".format(input_name = input_name)
		channel = load_h5_file(input_hd5_file, genome)
