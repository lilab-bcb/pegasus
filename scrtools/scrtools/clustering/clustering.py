...
from . import batch_correction, diffusion_map
...
def read_10x_h5_file(input_h5, genome):
	...
	adata = anndata.AnnData(X = X, 
							obs = {"obs_names" : attrs["barcodes"].decode()},
                            var = {"var_names" : attrs["gene_names"].decode(),
                             	   "gene_ids": attrs["genes"].decode()})
...
def cluster(input_name, output_name, **kwargs):
	...
	if not kwargs['input_h5ad']:
		adata = read_10x_h5_file(input_name + '_10x.h5', kwargs['genome'])
	else:
		adata = anndata.read_h5ad(input_name + '.h5ad')
